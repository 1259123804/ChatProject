...
//            NSMutableArray *groups = [NSMutableArray new];
            ...
            for (NSDictionary *dic in allGroups) {
                ...
                RCDGroupInfo *group = [[RCDGroupInfo alloc] init];
                group.groupId = [dic objectForKey:@"groupId"];
                group.groupName = [dic objectForKey:@"groupName"];
      ...
      success:^(id response) {
        ...
        if ([response[@"code"] integerValue] == 200) {
          ...
          for (NSDictionary *memberInfo in members) {
            ...
            member.userId = memberInfo[@"uid"];
            member.name = memberInfo[@"display_name"];
  ...
  [AFHttpTool uploadFile:image
      ...
      success:^(id response) {
        if ([response[@"key"] length] > 0) {
          ...
          NSString *QiNiuDomai = [[NSUserDefaults standardUserDefaults] objectForKey:@"QiNiuDomain"];
          NSString *imageUrl =
