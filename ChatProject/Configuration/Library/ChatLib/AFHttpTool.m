...
//    }
    baseURL = nil; //[KBonDay stringByAppendingString:url];
  //获得请求管理者
//  AFHTTPSessionManager *mgr =
//      [[AFHTTPSessionManager alloc]init];
//    NSString *tokenStr = [DEFAULTS objectForKey:@"token"];
//    [mgr.requestSerializer setValue:[@"Bearer "stringByAppendingString:tokenStr] forHTTPHeaderField:@"Authorization"];
...
// get group members by id bonday群成员信息
+ (void)getGroupMembersByID:(NSString *)groupID
                    success:(void (^)(id response))success
                    failure:(void (^)(NSError *err))failure {
  [AFHttpTool
      requestWihtMethod:RequestMethodTypeGet
                    url:[NSString stringWithFormat:@"campChatGroups/%@/users", groupID]
                 params:nil
                success:success
                failure:failure];
}
