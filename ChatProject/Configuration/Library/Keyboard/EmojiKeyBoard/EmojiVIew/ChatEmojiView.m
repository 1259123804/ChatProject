...
+ (instancetype)sharedEmojiView{
    static ChatEmojiView *emojiView = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
       
        emojiView = [[[self class] alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, ChatEmojiView_Hight)];
    });
...
- (void)addScroll:(CGRect)frame{
    _scroll = [[EmojiScrollView alloc]initWithFrame:frame];
...
- (void)addBottom:(CGRect)frame{
    ...
    sendB.frame = frame;
    [sendB setBackgroundImage:[UIImage imageFormColor:UIColorWithRGBA(235, 84, 4, 1) frame:sendB.bounds] forState:UIControlStateNormal];
...
- (void)sender:(IconButton*)sender{
    if (sender) {
        if(_selectIcon) _selectIcon.backgroundColor = [UIColor clearColor];
        _selectIcon = sender;
        _selectIcon.backgroundColor = UIColorWithRGBA(229,229,229,1);
    }
}
...
- (void)addEmjioWith:(Class)class{
    ...
    _scroll.contentSize = CGSizeMake(page_support*kScreenWidth, 0);
